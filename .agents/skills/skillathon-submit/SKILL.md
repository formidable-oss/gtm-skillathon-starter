...
1. Run `node .agents/skills/skillathon-submit/scripts/validate.mjs` from the repository root. If Node is missing, tell the participant to install it from <https://nodejs.org> (LTS) and continue once `node --version` works.
...
3. Commit everything: `git add -A && git commit -m "<short message>"`. Explain the command in one line if the participant is not a developer. Confirm `git status --porcelain` is empty.
4. Push: `git push -u origin HEAD`. If the push is rejected, fetch and resolve before continuing; never force-push.
...
6. Report the verdict verbatim. `ACCEPTED`: say the team is in and remind them they can resubmit after improvements until 20:30. `REJECTED`: fix what the comment lists, commit, push, and submit again. `DRY-RUN` (before 18:00): the pipeline works; submit again after 18:00.
...
- After 20:30 local time the script refuses. Do not look for a way around it; there is none.
